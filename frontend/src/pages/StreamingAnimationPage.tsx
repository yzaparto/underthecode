...
import { streamingArticles } from '../content/streaming/articles'
import { Prose } from '../components/InlineCode'

const TOTAL = streamingAnimations.length
...
export function StreamingAnimationPage() {
  const { step } = useParams<{ step: string }>()
  const index = Number(step) - 1

  if (isNaN(index) || index < 0 || index >= TOTAL) {
    return <Navigate to="/articles/python-streaming/1" replace />
  }

  const animation = streamingAnimations[index]
  const article = streamingArticles[index]
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <VoteButtons animationId={`python-streaming/${index + 1}`} />
      </div>
